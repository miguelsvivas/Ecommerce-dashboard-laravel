...
class ProductController extends Controller
{
      ...
      switch ($status) {
        case '0':
          ...
          break;
           ...
           break;
           case 'trash':
             $products = Product::with(['cat'])->onlyTrashed()->orderBy('id','desc')->paginate(10);
           break;
    ...
    public function postProductAdd(Request $request){
        ...
        return back()->withErrors($validator)->with('message','Se ha producido un error')->with('typealert','danger')->withInput();
          ...
          if($product->save()):
            ...
            return redirect('/admin/products')->withErrors($validator)->with('message','Guardado con exito')->with('typealert','success');
    endif;
       ...
       $rules =[
        ...
        return back()->withErrors($validator)->with('message','Se ha producido un error')->with('typealert','danger')->withInput();
          ...
          if($product->save()):
            ...
            return back()->withErrors($validator)->with('message','Actualizado con exito')->with('typealert','success');
    endif;
        ...
        return back()->withErrors($validator)->with('message','Se ha producido un error')->with('typealert','danger')->withInput();
         ...
         if($request->hasFile('file_image')):
          ...
          if($g->save()):
            ...
            return back()->withErrors($validator)->with('message','Imagen subida con exito')->with('typealert','success');
             endif;
    ...
    function getProductGalleryDelete($id, $gid){
      ...
      if($g->product_id != $id){
        ...
        return back()->with('message', 'La imagen borrada con exito.')->with('typealert' , 'success');
      endif;
       ...
       $rules =[
        ...
        return back()->withErrors($validator)->with('message','Se ha producido un error')->with('typealert','danger')->withInput();
              ...
              switch ($request->input('filter')):
                case '0':
                   $products = Product::with(['cat'])->where('name', 'LIKE', '%'.$request->input('search').'%')->where('status','0')->orderBy('id','desc')->get();               break;
                
                case '1':
                   $products = Product::with(['cat'])->where('code', 'LIKE', '%'.$request->input('search'))->orderBy('id','desc')->get();
                  break;
              endswitch;
    ...
    public function  getProductRestore($id){
        $p = Product::onlyTrashed()->where('id',$id)->first();
         if($p->restore()):
          return redirect('/admin/product/'.$p->id.'/edit')->with('message', 'Este producto se restauro con exito')->with('typealert' , 'success');
           endif;
